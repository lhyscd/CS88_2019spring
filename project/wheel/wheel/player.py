...
class ComputerPlayer(Player):
    ...
    def guess(self, board):
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        word_munch = WordMunch(self.possible_words)
        if self.skill == 1:
            word_munch.filter(lambda x: len(x) == len(board))
        if self.skill == 2:
            word_munch.filter(lambda x: len(x) == len(board))
            for idx in range(len(board)):
                if board.word()[idx] != '_':
                    word_munch.filter(lambda x: x[idx] == board.word()[idx])
        letter_set = word_munch.frequency()
        for letter in board.guesses():
            if letter in letter_set:
                del letter_set[letter]
        return(key_of_max(letter_set))
        # END

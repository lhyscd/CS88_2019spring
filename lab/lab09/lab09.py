...
class T88ble():
    ...
    def column(self, label):
        ...
        for idx in range(len(label)):
            if self.column_labels[idx] == label:
                col_idx = idx
                break
        for row in self.rows:
            r_list.append(row[col_idx])
        return r_list
    ...
    def with_column(self, label, values):
        ...
        """
        "*** YOUR CODE HERE ***"
        print(self.column_labels[:] + [label])
        for idx, row in enumerate(self.rows):
            print(row + [values[idx]])
    ...
    def select(self, labels):
        ...
        labels_idx = []
        for label in labels:
            labels_idx.append(self.column_labels.index(label))
        print(labels)
        tmp = []
        for row in self.rows:
            print([row[idx] for idx in labels_idx])
    ...
    def sort(self, label, descending=True):
        ...
        label_idx = self.column_labels.index(label)
        tmp = sorted(self.rows, key=lambda item: item[label_idx], reverse=descending)
        for i in tmp:
            print(i)

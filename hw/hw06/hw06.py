...
def div_interval(x, y):
    ...
    if reciprocal_y[0] * reciprocal_y[1] <= 0:
        return False
...
def sub_interval(x, y):
    ...
    """
    "*** YOUR CODE HERE ***"
    reciprocal_y = interval(0-upper_bound(y), 0-lower_bound(y))
...
def quadratic(x, a, b, c):
    ...
    if lbx < jzx < ubx:
        return interval(min([lby, uby, jzy]), max([lby, uby, jzy]))
    else:
        return interval(lby, uby)

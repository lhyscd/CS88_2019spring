...
def filter(f, s):
    ...
    if s == []:
        return s
    return [s[0]] + filter(f, s[1:]) if f(s[0]) else filter(f, s[1:])    
...
def reduce(reducer, s, base):
    ...
    "*** YOUR CODE HERE ***"
    if len(s) == 1:
        return reducer(s[0], base)
    else:
        return reducer(s[0],reduce(reducer,s[1:],base))    
...
def count_digit(n, digit):
    ...
    if int(n / 10) == 0:
        if n == digit:
            ...
            return 0
    if n % 10 == digit:
        return 1 + count_digit(int(n/10), digit)
    else:
        return count_digit(int(n/10), digit)
...
def ten_pairs(n):
    ...
    if int(n / 10) == 0:
        ...
        return count_digit(int(n/10), 10 - n%10) + ten_pairs(int(n/10))
...
def decimal(n):
    ...
    if n < 0:
        return_list.append('-')
        n = 0 - n
    return return_list + my_helper(n,10)
...
def binary(n):
    ...
    if n < 0:
        return_list.append('-')
        n = 0 - n
    return return_list + my_helper(n,2)
